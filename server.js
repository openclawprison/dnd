...
function ensureString(val) {
  ...
  if (val && typeof val === "object") { try { return JSON.stringify(val); } catch(e) { return String(val); } }
  return String(val || "");
}
...
function extractNarrative(text, depth) {
  ...
  if (t.startsWith("{")) {
    try {
      ...
      if (obj.narrative && typeof obj.narrative === "string") return extractNarrative(obj.narrative, depth + 1);
    } catch(e) {
...
function parseModelResponse(raw) {
  ...
  if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
    try { parsed = JSON.parse(cleaned.substring(firstBrace, lastBrace + 1)); } catch(e) { parsed = null; }
  }

  if (!parsed) {
    return {
      narrative: extractNarrative(raw, 0),
      roll: null, log: [], changes: {},
  ...
  if (parsed.log && Array.isArray(parsed.log)) {
    parsed.log = parsed.log.map(l => ({ type: ensureString(l.type || "info"), text: ensureString(l.text || "") })).filter(l => l.text);
  } else { parsed.log = []; }
  ...
  if (parsed.roll && typeof parsed.roll === "object" && parsed.roll.desc) {
    parsed.roll = { desc: ensureString(parsed.roll.desc), ability: ensureString(parsed.roll.ability || "str"), target: Number(parsed.roll.target) || 10, type: ensureString(parsed.roll.type || "check") };
  } else { parsed.roll = null; }

  // Final safety
  if (parsed.narrative.includes('"narrative"') || parsed.narrative.includes('"changes"')) {
    const m = raw.match(/"narrative"\s*:\s*"((?:[^"\\]|\\.)*)"/);
    if (m && m[1].length > 20) parsed.narrative = m[1].replace(/\\n/g, "\n").replace(/\\"/g, '"').replace(/\\\\/g, "\\");
  }
...
app.post("/api/summarize", async (req, res) => {
  ...
  const sysProm = `You are a story memory compressor for Tale Weavers, set in the Kingdom of Valdenmoor. Player: ${charName || 'adventurer'}.
RESPOND ONLY JSON (no markdown):
{"summary":"2-3 sentences","relationships":[{"name":"NPC","relation":"ally/enemy/neutral","notes":"brief"}],"agreements":["exact terms"],"keyEvents":["what happened"],"activeQuests":["status"],"secrets":["learned"],"threats":["danger"]}
NEVER drop existing memory unless resolved. Preserve agreements EXACTLY.`;

  const msgContent = recentMessages.map(m => `[${m.role}]: ${ensureString(m.text)}`).join("\n\n");
  const userMsg = `MEMORY:\n${currentMemory ? JSON.stringify(currentMemory) : "(empty)"}\n\nRECENT:\n${msgContent}\n\nUpdate.`;

  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01" },
      body: JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 600, system: sysProm, messages: [{ role: "user", content: userMsg }] }),
    });
    const data = await response.json();
